...
export default class onGoingJobList extends Component {
  ...
  doneCancle = () => {
    if (this.state.desc === '') {
      Alert.alert('Validation Error', 'Please Select one condition');
      return;
    }
    NetInfo.fetch().then((state) => {
      if (state.isConnected) {
        fetch(URL + '/order_Cancelled_status_by_order_id', {
          ...
          .then((result) => {
            ...
            if (result.length == 0) {
              ...
              Alert.alert('Error', 'Order Cancel not success');
            }
          })
          .catch((error) => {
            console.log(error);
  ...
  getMore = () => {
    switch (this.state.job_status) {
      case ' In Progress':
        this.getNext(
          '/get_next_inprogress_job_by_user_id',
          this.state.jobList[this.state.jobList.length - 1].order_id,
        );
        break;
      case 'Completed':
        this.getNext(
          '/get_next_completed_job_by_user_id',
          this.state.jobList[this.state.jobList.length - 1].order_id,
        );
        break;
      case 'All Jobs':
        this.getNext(
          '/get_next_all_job_by_user_id',
          this.state.jobList[this.state.jobList.length - 1].order_id,
        );
        break;
      default:
      ...
      .then((result) => {
        ...
        if (!result.error) {
          this.setState({
            jobList: [
              ...this.state.jobList,
              ...this.removeNull(result.order_details),
            ],
          });
        } else
          ToastAndroid.show(
                              ...
                              <View
                                ...
                                {items.item.user_role_id == '2' ? (
                                  ...
                                  <Text
                                    ...
                                    {items.item.parent_first_name}{' '}
                                    {items.item.parent_last_name}
                                  </Text>
                                )}
                              </View>
              ...
              <View
                ...
                {this.checkJobStatus(value.value) ? (
                  <View>
                    <TouchableOpacity
                      activeOpacity={2}
                      onPress={() => this.setStatus(value.value)}>
                      <Text
                        style={{
                          textAlign: 'center',
                          padding: 15,
